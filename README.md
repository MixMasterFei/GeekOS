...
2. **Daily cloud agent (9:00 Paris)**: a scheduled Claude Code routine reads the official article text the cron archives under `content/official/` (its sandbox cannot reach Blizzard directly), compares it with the Codex, calendar and manifest, and opens a pull request with the changes, each backed by a verbatim quote and file reference. Third-party sites are never used as sources. Nothing lands on `main` without review.
