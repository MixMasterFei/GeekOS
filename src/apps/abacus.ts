...
export const abacusApp: AppDef = {
  ...
  mount(ctx) {
    ...
    const upd = () => { small.textContent = expr; try { const v = safeEval(expr); big.innerHTML = coins ? toCoins(v) : (Number.isFinite(v) ? String(+v.toFixed(8)) : 'Fizzle'); } catch { big.textContent = 'Fizzle'; } };
