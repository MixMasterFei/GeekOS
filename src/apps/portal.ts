...
const MARKS = [
  { name: 'WoW: Forever (official)', url: 'https://worldofwarcraft.blizzard.com/en-us/forever' },
  { name: 'Warcraft Wiki', url: 'https://warcraft.wiki.gg/wiki/World_of_Warcraft:_Forever' },
  { name: 'Wikipedia: World of Warcraft', url: 'https://en.wikipedia.org/wiki/World_of_Warcraft' },
  { name: 'Battle.net developer portal', url: 'https://develop.battle.net/' },
];
...
export const portalApp: AppDef = {
  ...
  mount(ctx) {
    let url = ctx.args?.url ?? store.get('portal.last', MARKS[2].url);
    ...
    const marks = h('select', { class: 'input', style: { width: '210px' }, onchange: (e: Event) => { const v = (e.target as HTMLSelectElement).value; if (v) nav(v); (e.target as HTMLSelectElement).value = ''; } }, h('option', { value: '' }, 'Bookmarks…'), ...MARKS.map(m => h('option', { value: m.url }, m.name)));
