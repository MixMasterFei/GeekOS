 ...
 */
import { h, store, grantXp, esc, bus, type AppDef } from '../os/kernel';
import { CODEX, CODEX_COUNT } from '../data/codex';
import { icon } from '../os/icons';
import { achievements } from '../os/achievements';
import { sound } from '../os/sound';
import { launch } from '../os/shell';

export const codexApp: AppDef = {
  id: 'codex', name: 'Forever Codex', subtitle: 'Everything announced, in one book', icon: 'codex', category: 'adventure', width: 940, height: 600, noScroll: true,
  mount(ctx) {
    ...
    const progress = () => { const p = h('div', { class: 'bar gold', style: { marginTop: '6px' } }, h('i', { style: { width: (100 * read.size / CODEX_COUNT) + '%' } }), h('span', {}, `${read.size} / ${CODEX_COUNT} entries read`)); return p; };
    const renderNav = () => {
      nav.innerHTML = '';
      const s = h('input', { class: 'input', placeholder: 'Search the Codex…', value: q, oninput: (e: Event) => { q = (e.target as HTMLInputElement).value.toLowerCase(); entryId = null; renderPage(); } });
      ...
      nav.append(h('div', { class: 'dim small', style: { marginTop: 'auto', padding: '8px 4px' } }, 'Compiled from Blizzard\'s public announcements of September 12–13, 2026.'));
    };
    const markRead = (id: string) => { if (!read.has(id)) { read.add(id); store.set('codex.read', [...read]); grantXp(8, 'codex'); bus.emit('codex:read', id); if (read.size >= CODEX_COUNT) achievements.unlock('codex-all'); renderNav(); } };
    const renderPage = () => {
      page.innerHTML = '';
      if (q) {
        const hits = CODEX.flatMap(s => s.entries.map(e => ({ s, e }))).filter(({ e }) => (e.name + ' ' + (e.sub ?? '') + ' ' + e.text).toLowerCase().includes(q));
        page.append(h('div', { class: 'eyebrow' }, `${hits.length} result${hits.length !== 1 ? 's' : ''}`));
        hits.forEach(({ s, e }) => page.append(card(e, s.id)));
        return;
      }
      const sec = CODEX.find(s => s.id === secId)!;
      if (entryId) {
        const e = sec.entries.find(x => x.id === entryId)!; markRead(e.id);
        ...
        page.append(h('div', { class: 'frame', style: { padding: '0 0 22px', animation: 'riseIn .25s ease', overflow: 'hidden' } },
          ...
          h('div', { class: 'row', style: { margin: '18px 26px 0', justifyContent: 'space-between' } },
            (() => { const i = sec.entries.indexOf(e); const p = sec.entries[i - 1]; return h('button', { class: 'btn sm ghost', disabled: !p || undefined, onclick: () => { entryId = p.id; renderPage(); } }, p ? '‹ ' + p.name : '‹'); })(),
            sec.id === 'zones' ? h('button', { class: 'btn sm', onclick: () => launch('atlas', { zone: e.id }) }, 'Show on Atlas') : sec.id === 'editions' && e.id === 'dates' ? h('button', { class: 'btn sm', onclick: () => launch('calendar') }, 'Open Calendar') : h('span'),
            (() => { const i = sec.entries.indexOf(e); const n = sec.entries[i + 1]; return h('button', { class: 'btn sm ghost', disabled: !n || undefined, onclick: () => { entryId = n.id; renderPage(); } }, n ? n.name + ' ›' : '›'); })())));
        return;
      }
      page.append(h('div', { style: { marginBottom: '14px' } }, h('div', { class: 'eyebrow' }, 'Forever Codex'), h('h2', { class: 'display', style: { fontSize: '28px' } }, sec.title), h('div', { class: 'dim' }, sec.blurb)));
    ...
    const card = (e: any, sid: string) => {
      const sec = CODEX.find(s => s.id === sid)!;
      const el = h('div', { class: 'frame', style: { padding: '0', cursor: 'pointer', display: 'flex', flexDirection: 'column', opacity: read.has(e.id) ? .85 : 1, overflow: 'hidden' } },
      ...
      el.addEventListener('click', () => { secId = sid; entryId = e.id; q = ''; sound.click(); renderNav(); renderPage(); page.scrollTop = 0; });
      return el;
    };
    renderNav(); renderPage();
    ctx.setStatus(`<span>${CODEX.length} chapters · ${CODEX_COUNT} entries</span><span class="dim">Read them all for an achievement</span>`);
    void esc;
  },
};
