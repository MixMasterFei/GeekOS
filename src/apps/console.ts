...
export const consoleApp: AppDef = {
  ...
  mount(ctx) {
    ...
    const cmds: Record<string, { help?: string; run: (args: string, raw: string) => void }> = {
      ...
      open: { help: 'Open an app: /open <name>', run(a) { const q = a.trim().toLowerCase(); if (!q) { say('err', 'Usage: /open <name>. Try /apps.'); return; } const all = appList().filter(x => !x.hidden); const app = all.find(x => x.id === q || x.name.toLowerCase() === q) ?? (() => { const ms = all.filter(x => x.id.startsWith(q) || x.name.toLowerCase().startsWith(q)); if (ms.length === 1) return ms[0]; const ws = all.filter(x => x.name.toLowerCase().includes(q) || x.id.includes(q)); if (ws.length === 1) return ws[0]; if (ws.length > 1) { say('err', `Which one? ${ws.map(x => '<span class="gold">' + x.id + '</span>').join(', ')}`); return null; } return undefined; })(); if (app === null) return; if (!app) { say('err', `No such app: ${esc(a)}. Try /apps.`); sound.error(); return; } launch(app.id); sys(`Casting <b>${esc(app.name)}</b>…`); } },
      ...
      who: { help: 'Look someone up: /who <name>', run(a) { const q = a.trim().toLowerCase(); if (!q) { sys(`1 player total: ${esc(u.name)} — Level ${u.level} ${u.race} ${u.cls} — Zone: GeekOS Desktop`); return; } if (WHO[q]) { sys(esc(WHO[q])); if (q === 'thrall') achievements.unlock('who-thrall'); } else sys(`0 players total matching "${esc(a.trim())}". They may be in another realm.`); } },
      ...
      cat: { help: 'Read a scroll: /cat <name>', run(a) { if (!a.trim()) { say('err', 'Usage: /cat <name>'); return; } const n = fs.find(a.trim()).find(x => x.type === 'text' || x.type === 'letter'); if (!n) { say('err', 'No such scroll.'); return; } say('say', `<pre style="white-space:pre-wrap;margin:0">${esc(n.content ?? '')}</pre>`); if (n.id === 'scarlet') achievements.unlock('secret-file'); } },
      ...
      g: { help: 'Guild chat', run(a) { if (!a.trim()) { say('err', 'Usage: /g <message>'); return; } say('guild', `[Guild] [${esc(u.name)}]: ${esc(a)}`); setTimeout(() => say('guild', `[Guild] [${rand(['Grimtotem', 'Pixelbrand', 'Sundershot', 'Velaria'])}]: ${rand(['lol', 'gz', 'anyone for Hall of Thanes?', 'Forever hype', 'need a healer for Barrow Deeps', 'brb, camping'])}`), 900 + Math.random() * 1500); } },
      w: { run(a) { const [to, ...rest] = a.trim().split(/\s+/); if (!to || !rest.join(' ').trim()) { say('err', 'Usage: /w <name> <message>'); return; } say('whisper', `To [${esc(to)}]: ${esc(rest.join(' '))}`); setTimeout(() => { sound.whisper(); say('whisper', `[${esc(to)}] whispers: ${rand(['?', 'who dis', 'not now, raiding', 'mrgl', 'meet me in the Riverglades'])}`); }, 1200); } },
