...
export const chessApp: AppDef = {
  ...
  mount(ctx) {
    ...
    side.append(h('div', { class: 'eyebrow' }, 'Karazhan Chess'), status, h('div', { class: 'dim small' }, 'You play the Alliance (white). The tower plays the Horde.'), logEl,
      h('div', { class: 'row' }, h('button', { class: 'btn sm gold', onclick: () => { s = initial(); sel = null; last = null; over = ''; log.length = 0; logEl.innerHTML = ''; render(); } }, 'New game'), h('button', { class: 'btn sm ghost', onclick: () => { if (thinking) return; const n = s.turn === 'w' ? 2 : 1; if (history.length >= n) { s = history[history.length - n]; history.splice(-n); log.splice(-n); last = null; sel = null; moves = []; over = ''; renderLog(); render(); } } }, 'Undo')));
