...
export const scribeApp: AppDef = {
  ...
  mount(ctx) {
    ...
    const upd = () => { renameBtn.toggleAttribute('disabled', !file || readonly); ctx.setTitle(`Scribe — ${name()}${dirty ? ' •' : ''}`); const words = ta.value.trim() ? ta.value.trim().split(/\s+/).length : 0; ctx.setStatus(`<span>${words} words</span><span>${ta.value.length} runes</span><span class="dim">${readonly ? 'Read only' : dirty ? 'Unsaved' : 'Saved'}</span><span class="dim" style="margin-left:auto">Ctrl+S to save</span>`); };
    const save = async () => {
      if (readonly) return;
